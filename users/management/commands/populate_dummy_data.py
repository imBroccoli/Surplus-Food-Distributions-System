...
from transactions.models import FoodRequest, Transaction, DeliveryAssignment, Rating
...
class Command(BaseCommand):
    ...
    def create_food_listings(self):
        ...
        for i in range(2):
            ...
            # Create a rating for the completed transaction
            rater = random.choice([requester, listing.supplier])
            rated_user = listing.supplier if rater == requester else requester
            Rating.objects.create(
                transaction=transaction,
                rater=rater,
                rated_user=rated_user,
                rating=random.randint(3, 5),
                comment="Auto-generated rating for analytics."
            )
            ...
            for _ in range(random.randint(2, 4)):
                try:
                    ...
                    if request.status in ["APPROVED", "COMPLETED"]:
                        ...
                        # Create a rating for completed transactions
                        if status == "COMPLETED":
                            rater = random.choice([requester, listing.supplier])
                            rated_user = listing.supplier if rater == requester else requester
                            Rating.objects.create(
